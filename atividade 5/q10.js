...
function tipoPulverizacao(tipo){
    switch (tipo) {
        case '1':
            return 50
            break;
   ...
   const area = parseInt(prompt('Quantos acres serão pulverizados?'))
   const tipo1 = tipoPulverizacao((prompt(`Escolha o tipo de pulverização:
    Tipo 1 – ervas daninhas R$ 50,00 por acre;
    Tipo 2 – gafanhotos R$ 100,00 por acre;
    Tipo 3 – broca R$ 150,00 por acre;
    Tipo 4 – todos acima R$ 250,00 por acre.`)))
    console.log(tipo1)
   let valor = tipo1 * area
   let desconto = 0
   let descontoMaior = 0
   if(area > 1000 && valor > 750){
        desconto = valor * 0.05
        valor -= desconto
        descontoMaior = valor - 750
        if (descontoMaior > 0){
            let valorFinal = descontoMaior
            descontoMaior *= 0.1
            valorFinal -= descontoMaior
            alert(`Valor Final para pagamento com 2 descontos ${valorFinal}`)
        }
    } else if (area < 1000 && valor > 750){
        desconto = valor * 0.1
        valor -= desconto
        alert(`Valor Final para pagamento com 10% de desconto: ${valor}`)
    } else {
        alert(`Valor Final sem desconto: ${valor}`)
    }
